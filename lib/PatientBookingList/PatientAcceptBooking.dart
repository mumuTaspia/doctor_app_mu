...
class PatientAcceptBookingList extends StatefulWidget {
  @override
  _BookingListByDoctorState createState() => _BookingListByDoctorState();
}

class _BookingListByDoctorState extends State<PatientAcceptBookingList> {
  ...
  Future getBookingList() async {
    ...
    print(userid);
    final responce = await http.post("https://doctor-api.appstic.xyz/appointmentbystatuspatient/accept/"+userid);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
     drawer: PatientDrawer(),
      appBar: AppBar(title: Text("Accept Booking List"),),
      body: _isLoading ? Container(
        ...
        child:bookingList.length==0 ? Container(
            height: MediaQuery.of(context).size.height,
            width: MediaQuery.of(context).size.width,
            child: Center(child: Text("No Accept Booking"))) : ListView.builder(
